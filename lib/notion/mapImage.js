...
//   // 定义请求的 URL
//   const url = 'https://api.unsplash.com/photos/random?client_id=r5h81RZMjZdoOcLcOPZwjbdJ-t-le4yMnfFWORuBEb8&count=1&orientation=landscape';
...
//         return data[0].urls.small_s3; // 获取图片的 url
//       } else {
//         return 'https://notion.zsccy.top/favicon.ico'; // 没有照片时的返回值
//       }
//     } else {
//       return `https://notion.zsccy.top/favicon.ico`; // 请求失败时的返回值
//     }
//   } catch (error) {
//     return `https://notion.zsccy.top/favicon.ico`; // 处理异常情况
//   }
// }
...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    if (
      ...
      // 如果 ret 以 source.unsplash.com 开头
      if (ret.indexOf('https://source.unsplash.com') === 0) {
        const width = 500;
        const height = 240;
        ret = `${ret.trim()}/${width}x${height}${separator}t=${block.id}`;
      }else{
